...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  site_name='$sitename', tx_site_type='$txSiteType', city_region='$region', department='$department', site_auto_status='$siteAutoStatus',
    site_status='$siteStatus', class='$class' WHERE site_id='$siteId'";

  if ($conn->query($query) === TRUE) {
    session_start();
    // $_SESSION['message'] = "Loan Updated successfully";
    // echo "<script>window.location.href = 'class-details.php';</script>";
    echo "<script>window.location.href = 'class-details.php?class=" . urlencode($class) . "';</script>";
    exit;
  } else {
