<?php
session_start();
if (!isset($_SESSION["username"])) {
    header('location: ../index.php');
}
?>

<?php
// Connect to the database
include 'connection/db_connection.php';

// Get the age category from URL parameters
$age_category = isset($_GET['category']) ? $_GET['category'] : '';

// Fetch all site_ids and count their occurrences
$site_id_counts = [];
$sql_count = "SELECT site_id, COUNT(*) as count FROM generators GROUP BY site_id";
$count_result = $conn->query($sql_count);

if ($count_result) {
    while ($row = $count_result->fetch_assoc()) {
        $site_id_counts[$row['site_id']] = $row['count'];
    }
} else {
    die("Query failed: " . $conn->error);
}

// Build the SQL query with direct age calculation in the query
$sql = "SELECT
    g.generator_id AS generator_id,
    g.site_id AS site_id,
    g.status AS status,
    g.installation_date AS installation_date,
    g.actual_run_hours AS actual_run_hours,
    g.latest_actual_run_hrs AS latest_actual_run_hrs,
    s.secondary_id AS secondary_id,
    s.site_name AS site_name,
    TIMESTAMPDIFF(MONTH, g.installation_date, CURRENT_DATE()) AS age_in_months
FROM 
    generators g
JOIN 
    sites s ON g.site_id = s.primary_id";

// Apply filtering based on age category
if ($age_category != '') {
    switch ($age_category) {
        case '0 - 6 Months':
            $sql .= " WHERE TIMESTAMPDIFF(MONTH, g.installation_date, CURRENT_DATE()) <= 6"; // 0-6 months
            break;
        case '6 - 12 Months':
            $sql .= " WHERE TIMESTAMPDIFF(MONTH, g.installation_date, CURRENT_DATE()) > 6 AND TIMESTAMPDIFF(MONTH, g.installation_date, CURRENT_DATE()) <= 12"; // 6-12 months
            break;
        case '1 - 2 Years':
            $sql .= " WHERE TIMESTAMPDIFF(MONTH, g.installation_date, CURRENT_DATE()) > 12 AND TIMESTAMPDIFF(MONTH, g.installation_date, CURRENT_DATE()) <= 24"; // 12-24 months
            break;
        case '2 - 5 Years':
            $sql .= " WHERE TIMESTAMPDIFF(MONTH, g.installation_date, CURRENT_DATE()) > 24 AND TIMESTAMPDIFF(MONTH, g.installation_date, CURRENT_DATE()) <= 60"; // 24-60 months
            break;
        case '5+ Years':
            $sql .= " WHERE TIMESTAMPDIFF(MONTH, g.installation_date, CURRENT_DATE()) > 60"; // More than 60 months
            break;
    }
}

$result = $conn->query($sql);

if (!$result) {
    die("Query failed: " . $conn->error);
}
?>

<style>
    i {
        font-size: 20px;
    }

    .table a {
        padding: 1px 1px 1px 1px;
    }

    input[type="search"] {
        width: 100%;
        padding: 5px 5px 5px 5px;
        border: 1px solid #ccc;
        border-radius: 4px;
        resize: vertical;
    }

    .genid-deco:hover {
        text-decoration: underline;
        color: green;
    }
</style>

<script>
    // Get the reference to the div element
    var divElement = document.getElementById("message-container");

    // Function to remove the div from the DOM
    function removeDiv() {
        if (divElement) {
            divElement.parentNode.removeChild(divElement);
        }
    }

    // Add event listener only if the element exists
    document.addEventListener("DOMContentLoaded", function() {
        if (divElement) {
            // Add the 'removeDiv' function as an event listener for 'transitionend' event
            divElement.addEventListener("transitionend", removeDiv);

            // Trigger the fade-out effect after a short delay
            setTimeout(function() {
                divElement.classList.add("fade-out");
            }, 2000); // Delay of 2 seconds (2000 milliseconds)
        }
    });
</script>

<?php include 'header_aside.php'; ?>

<!-- main -->
<main id="main" class="main">

    <?php if (isset($_SESSION['message'])) { ?>
        <div class="container" id="message-container">
            <div class="row">
                <div class="col-md-12">
                    <div class="alert alert-success d-flex justify-content-between align-items-center">
                        <?php echo $_SESSION['message']; ?>
                        <!-- add button for removing the alert message -->
                        <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                    </div>
                </div>
            </div>
        </div>
    <?php }
    unset($_SESSION['message']); ?>

    <section>
        <div class="container">
            <div class="row">
                <div class="col-md-12">
                    <h2 class="text-center">List of Generators: <?php echo htmlspecialchars($age_category); ?></h2>
                    <hr>

                    <button onclick="window.location.href='export-generators.php?age_category=<?php echo urlencode($age_category); ?>&format=csv'" class="btn btn-primary">Download as CSV</button>
                    <button onclick="window.location.href='export-generators.php?age_category=<?php echo urlencode($age_category); ?>&format=excel'" class="btn btn-secondary">Download as Excel</button>
                    <hr>
                </div>

                <table class="table datatable table-striped">
                    <thead class="thead-dark">
                        <tr>
                            <th scope="col">Generator ID</th>
                            <th scope="col">Site Name</th>
                            <th scope="col">Projected RHrs</th>
                            <th scope="col">Age (Months)</th>
                            <th scope="col">Actual RHrs</th>
                            <th scope="col">Latest Actual RHrs</th>
                        </tr>
                    </thead>
                    <tbody>
                        <?php
                        $index = 1;
                        while ($generator = $result->fetch_assoc()) {
                            $site_id = $generator['secondary_id'];
                            $count = $site_id_counts[$generator['site_id']] ?? 1; // Default to 1 if not found

                            // Determine the generator ID format
                            if ($count > 1) {
                                // If site_id appears more than once, use _G1, _G2, etc.
                                static $site_id_index = []; // Track index for each site_id
                                if (!isset($site_id_index[$site_id])) {
                                    $site_id_index[$site_id] = 1; // Initialize index for this site_id
                                }
                                $generator_id = $site_id . '_G' . $site_id_index[$site_id]++;
                            } else {
                                // If site_id appears only once, use _G1
                                $generator_id = $site_id . '_G1';
                            }

                            // Use age_in_months directly from the query result
                            $genset_age = $generator['age_in_months'] / 12;
                            $genset_age = round($genset_age, 1);

                            // Calculate projected runtime hours (based on installation date to current date)
                            $installation_date = $generator['installation_date'];
                            date_default_timezone_set('Africa/Nairobi');
                            $current_time = new DateTime();
                            $created_time = new DateTime($installation_date);
                            $interval = $current_time->diff($created_time);
                            $cumulative_hours = ($interval->days * 24) + $interval->h;
                        ?>
                            <tr>
                                <td scope="row">
                                    <form action="generator-details.php?generator_id=<?php echo htmlspecialchars($generator['generator_id']); ?>" method="POST" style="display:inline;">
                                        <?php $real_generator_id = $generator_id ?>
                                        <input type="hidden" name="real-generator-id" value="<?php echo htmlspecialchars($real_generator_id); ?>">
                                        <button type="submit" title="More details" class="text-dark genid-deco" style="border: none; background: none; padding: 0;">
                                            <?php echo htmlspecialchars($real_generator_id); ?>
                                        </button>
                                    </form>
                                </td>
                                <td><?php echo $generator['site_name']; ?></td>
                                <td>
                                    <?php if (strtolower($generator['status']) === strtolower("Not Operational")) {
                                        echo "Stopped";
                                    } else {
                                        echo $cumulative_hours ?? "Not available";
                                    } ?>
                                </td>
                                <td><?php echo $genset_age; ?></td>
                                <td><?php echo $generator['actual_run_hours']; ?></td>
                                <td>
                                    <?php echo htmlspecialchars($generator['latest_actual_run_hrs']); ?>
                                    <a href="update-run-hour.php?generator_id=<?php echo htmlspecialchars($generator['generator_id']); ?>"
                                        title="Update" class="text-success"><i class="bi bi-pencil-square"></i></a>
                                </td>
                            </tr>
                        <?php } ?>
                    </tbody>
                </table>
            </div>
        </div>
    </section>
</main><!-- End #main -->

<!-- ======= Footer ======= -->
<?php include 'footer.php'; ?>